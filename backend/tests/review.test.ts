...
describe("Integration Test - Review API", () => {
  ...
  beforeAll(async () => {
    ...
    const loginRes1 = await request(app).post("/api/auth/login").send({ email: testUser1.email, password: testUser1.password });
    token1 = loginRes1.body.token;

    const loginRes2 = await request(app).post("/api/auth/login").send({ email: testUser2.email, password: testUser2.password });
    token2 = loginRes2.body.token;

    // 2. Create a test cafe directly in db
    const { data: cafe } = await supabase.from('cafes').insert([testCafe]).select('id').single();
    cafeId = cafe!.id;
  });
